...
export const WindBlownHair = () => {
    const curves = [];
    const hairLength = 10; // Length of the hair
    const windStrength = 2; // Strength of the wind effect
    const segments = 5; // Number of segments in the hair

    for (let i = 0; i < segments; i++) {
        // Calculate the position of each segment
        const segmentHeight = hairLength * (i / segments);
        const nextSegmentHeight = hairLength * ((i + 1) / segments);

        // Start and end points of the curve
        const startPoint = new THREE.Vector3(0, segmentHeight, 0);
        const endPoint = new THREE.Vector3(0, nextSegmentHeight, 0);

        // Control points to simulate the wind effect
        const controlPoint1 = new THREE.Vector3(
            windStrength * Math.sin(i), segmentHeight + (nextSegmentHeight - segmentHeight) / 3, 0
        );
        const controlPoint2 = new THREE.Vector3(
            windStrength * Math.sin(i + 0.5), segmentHeight + 2 * (nextSegmentHeight - segmentHeight) / 3, 0
        );
